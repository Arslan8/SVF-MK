...
string  argToBridge(CallInst * ci, int argnum, Value ** v, Value ** sizeInt, Value * arg=NULL) {
		if (arg==NULL) {
			arg = ci->getArgOperand(argnum);
		}
		string args;
		...
		if (arg->getType()->isIntegerTy()) {
			...
			args = "p";
                IRBuilder<> Builder(ci);
			// Create two zero indices
			std::vector<Value*> indices(1, ConstantInt::get(Builder.getInt64Ty(), 0));
			//arg->getType()->getArrayElementType()
			//Value* elementPtr = Builder.CreateInBoundsGEP(arg->getType(), arg, indices, "element_ptr");
			arg= Builder.CreateExtractValue(arg, {0}, "first_element");
			return argToBridge(ci, argnum, v, sizeInt, arg);
		}
...
int compartmentalize(char * argv[]) {
		...
		for (auto G = svfModule->global_begin(), E = svfModule->global_end(); G != E; ++G) {
				...
				if ((*glob)->getName().str() == "llvm.used" || (*glob)->getName().str() == "_shared_region" || (*glob)->getSection().str().find(isr) != std::string::npos
								...
								(*glob)->getSection().str().find(syscall) != std::string::npos) {
		...
		for (auto F = ll_mod->begin(), E=ll_mod->end(); F!= E; ++F) 
		{
				...
				if (fun->getSection().str() == "llvm.used" || fun->getSection().str() == "_shared_region"
								...
								fun->getSection().str().find(syscall) != std::string::npos) {
                        ignoreList<<fun->getName().str()<<endl;
                        continue;
                }
				if (fun->isIntrinsic ()) {
	...
	//C++ Analysis code to walk vtables
		...
		for (auto G = svfModule->global_begin(), E = svfModule->global_end(); G != E; ++G) {
				...
				if (auto go= dyn_cast<llvm::GlobalVariable> (*glob)) {
						...
						string isr = "isr_vector";
                        if (go->getSection().str().find(isr) != std::string::npos) {
                            continue;
                        }
		...
		for (SVFModule::llvm_iterator F = svfModule->llvmFunBegin(), E = svfModule->llvmFunEnd(); F != E; ++F) {
				...
				string syscall = "system_calls";
				if (fun->getSection().str().find(syscall) != std::string::npos) {
                        continue;
                }
